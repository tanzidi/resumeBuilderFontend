...
const SVGIcon = (pathD: string) => {
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" width="16px" height="16px" fill="white"> <path d="${pathD}" /> </svg>`;
};
...
const component = (type: IconType, url: string, notLinked?: boolean) => {
  const pathD = TYPE_TO_PATH_D[type];
  const link = type === "email" ? "mailto:" + url : url;
  if (notLinked) {
    return `<div class="icon-set">${SVGIcon(pathD)}<p>${url}<p></div>`;
  }
  return `<div class="icon-set">${SVGIcon(
    pathD
  )}<a href=${link} >${url}</a></div>`;
};
...
export const iconWithLink = (
  ...
  const icon = type ? type : iconName(url);
  if (isEmail) {
    return component("email", url);
  } else return component(icon, url, notLinked);
};
